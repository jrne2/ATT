...
def transcribe_audio(audio_bytes, language_code='en-US'):
    ...
    try:
        ...
        start_time = time.time(); timeout_seconds = 60
        while True:
            if time.time() - start_time > timeout_seconds:
                print(f"Transcribe 작업 타임아웃 ({timeout_seconds}초)")
                transcript_text = "[음성 인식 시간 초과]"; break 
            status = transcribe_client.get_transcription_job(TranscriptionJobName=job_name)
            job_status = status['TranscriptionJob']['TranscriptionJobStatus']
            if job_status in ['COMPLETED', 'FAILED']: break 
            print(f"Transcribe 작업 진행 중... ({job_status})"); time.sleep(3) 
...
# --- 👇 여기가 수정된 get_ai_response 함수입니다 (잘못된 주석 제거) ---
...
- **Tone:** Warm, empathetic, positive, casual. Asks questions.
- **[CRITICAL RULE - INTENT]:** Recommendations MUST maintain the user's original intent. (e.g., "Hello" -> "Hey! So great to hear from you!")."""
    elif persona == "자신감 넘치는 비즈니스 리더":
         persona_specific_instructions = f"""
**Special Instructions for 'Business Leader Persona':**
- **Tone:** Clear, concise, confident, decisive, results-oriented.
- **[CRITICAL RULE - INTENT]:** Recommendations MUST maintain the user's original intent. (e.g., "Hello?" -> "Good morning. We can start.")."""
...
**CRITICAL RULE 1: MAINTAIN USER'S ORIGINAL INTENT AND SCALE. THIS IS THE MOST IMPORTANT RULE OF ALL.**
...
Assistant:"""
    ...
    try:
        ...
        if "|||SCORE:::" in full_response_text:
            parts = full_response_text.split("|||SCORE:::", 1);
            if len(parts) == 2:
        ...
        if "FEEDBACK:::" in raw_main_output: is_feedback = True; main_output_text = raw_main_output.split("FEEDBACK:::", 1)[-1].strip()
        elif "RESPONSE:::" in raw_main_output: is_feedback = False; main_output_text = raw_main_output.split("RESPONSE:::", 1)[-1].strip()
